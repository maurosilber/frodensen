...
This lower bound is easily established when considering the likelihood function defined by eq. (8.1).
...
Given an estimator $t$ of some function of $\theta$, say T($\theta$),
we define its bias $b(\theta)$ by the relation (compare eq. (8.4)),
%
\begin{equation}
    E(t)
    = \int \ldots \int t(x_1, x_2, \ldots, x_n) L(x_1, x_2, \ldots, x_n | \theta) dx_1 dx_2 \ldots dx_n
    = \tau(\theta) + b(\theta)
\end{equation}
%
With the assumption that the range of $x$ is independent of $\theta$,
the differentiation of eq. (8.5) with respect to $\theta$ gives
%
\begin{equation}
...
\end{equation}
%
Multiplying eq. (8.8) by $\tau(\theta)$
and subtracting the result from eq. (8.6) leads to
%
\begin{equation}
...
%
\begin{equation}
    \left( \int \ldots \int [t - \tau(\theta)]^2 L d\vec{x} \right)
...
\end{equation}
%
The lower limit of the variance implied by eqs. (8.11), (8.13) is called the minimum variance bound, MVB,
...
if the Schwarz inequality applied to eq. (8.9) becomes an equality.
...
\end{equation}
%
Efficient estimators exist only for the limited class of problems for which the likelihood function satisfies the condition (8.14).
...
and the Cramer-Rao inequalities (8.11) and (8.13) become, respectively,
%
\begin{align}
    V(t)
    &\geq \frac
...
\textbf{Exercise 8.3:}
Prove eq. (8.12).
(Hint: Differentiate eq. (8.8) with respect to $\theta$).
